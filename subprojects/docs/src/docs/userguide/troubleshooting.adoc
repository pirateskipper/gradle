...
This may happen when the connection request comes from an IP address that Gradle labels as external (and rejects due to security concerns). We have observed this scenario in some NAT network setup. The solution is to adjust the network configuration such that local network connections are not routed through external addresses.
